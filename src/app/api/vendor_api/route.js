...
export async function POST(req){
  ...
  const{vendorName,email,phone ,gstNumber, city, status,address}= rest;

  try{


           const my_gst= 'select * from vendors where gst_number= $1';
          ...
          return NextResponse.json({message:"This GST Number is already exist"}, {status:409});
           ...
           const my_phone= 'select * from vendors where phone_number= $1';
    ...
    const my_query = 'Insert into vendors (vendor_name,email,phone_number,gst_number,city,status,address) values($1,$2,$3,$4,$5,$6,$7)';

    const value= [vendorName,email,phone,gstNumber,city,status,address];
